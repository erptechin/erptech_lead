...
const fields_list = ['customer', 'custom_agent', 'custom_lead', 'custom_plot_detail', 'items']
...
export default function AddEditFrom() {
      ...
      if (customer) {
        try {
          ...
          const response = await getCustomData({
            url: `erptech_lead.api.custom.get_customer_address?customer_name=${customer}`
          });
  ...
  const onSubmit = async (formData) => {
    if (id) {
      const response = await getCustomData({
        url: `erptech_lead.api.doctype.update_data?doctype=${doctype}&name=${id}&update_fields=${JSON.stringify([{ 'docstatus': 0 }])}`
      });
      if (response.success) {
