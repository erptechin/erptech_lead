...
class CODManagement(Document):
	def on_update(self):
		"""
		Handle Management document updates
		...
		if approval_manager:
			try:
				...
				if hasattr(self, '_doc_before_save') and self._doc_before_save:
					...
					previous_approval_manager = frappe.db.get_value("Management", self.name, "approval_manager")
				...
				if previous_approval_manager and previous_approval_manager != approval_manager:
					try:
						remove_share("Management", self.name, previous_approval_manager)
						frappe.logger().info(
							f"Removed share access for user {previous_approval_manager} from Management {self.name}"
						)
					except Exception as e:
						# Share might not exist, which is fine
						frappe.logger().debug(
							f"Could not remove share for user {previous_approval_manager} from Management {self.name}: {str(e)}"
						)
				
				# Add share access to new approval manager
				user = frappe.get_doc("User", approval_manager)
				add_share("Management", self.name, user.name, write=1, share=1)
				frappe.msgprint(
					frappe._("Management access has been granted to {0}").format(user.name)
				)
			except Exception as e:
				frappe.log_error(
					f"Error managing share for user {approval_manager} on Management {self.name}: {str(e)}",
					"Management Share Error"
				)
		...
		if current_status == "Approve":
			...
			if hasattr(self, '_doc_before_save') and self._doc_before_save:
				...
				previous_status = frappe.db.get_value("Management", self.name, "status")
			
			# Only process if previous status was NOT "Approve" (to avoid duplicate processing)
			if previous_status != "Approve":
				try:
					# Set status to "Active"
					frappe.db.set_value("Plot Detail", self.plot_detail, "status", "Active")
					
					
					# Set renew_date to 1 year from current date
					renew_date = add_years(today(), 1)
					frappe.db.set_value("Plot Detail", self.plot_detail, "renew_date", renew_date)
					
					# Get required fields for Sales Order
					customer = self.get("customer")
					lead = self.get("lead")
					plot_detail = self.get("plot_detail")
					policy_amount = self.get("policy_amount") or 0
					
					if not customer:
						frappe.log_error(
							f"Customer is required to create Sales Order for Management {self.name}",
							"Management Sales Order Error"
						)
						return
					
					if not lead:
						frappe.log_error(
							f"Lead is required to create Sales Order for Management {self.name}",
							"Management Sales Order Error"
						)
						return
					...
					if not agent:
						frappe.log_error(
							f"Lead {lead} does not have an assigned user (agent) for Management {self.name}",
							"Management Sales Order Error"
						)
						return
					...
					sales_order.custom_plot_detail = plot_detail
					...
					frappe.log_error(
						f"Error processing approved Management {self.name}: {str(e)}",
						"Management Approval Error"
					)
					frappe.msgprint(
